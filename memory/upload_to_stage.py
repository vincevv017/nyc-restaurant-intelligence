...
upload_to_stage.py
──────────────────────────────────────────────────────────────────────────────
Uploads memory/ app files to the Snowflake SiS stage using snowflake-connector-
python (already installed in the cortex venv). Uses key-pair auth — password
auth triggers MFA on paid accounts.
...
STAGE = "@RESTAURANT_INTELLIGENCE.RAW.STREAMLIT_MEMORY_STAGE"

# Files to upload — paths relative to repo root.
# streamlit_app.py is self-contained: CORTEX_TOOLS, CORTEX_TOOL_RESOURCES,
# SYSTEM_PROMPT, and _parse_sse are inlined. No cortex_agent.py on the stage.
REPO_ROOT = Path(__file__).parent.parent
FILES = [
    REPO_ROOT / "memory" / "streamlit_app.py",
    REPO_ROOT / "memory" / "memory_manager.py",
    REPO_ROOT / "memory" / "response_parser.py",
    REPO_ROOT / "memory" / "environment.yml",
    REPO_ROOT / "map"    / "map_engine.py",     # shared pydeck builder
]
